...
def setup_cjk_font() -> None:
    """配置 matplotlib 中文字体（macOS PingFang SC 优先）与负号显示。

    在绘图前调用一次即可：:

        from src.utils.plot import setup_cjk_font
        setup_cjk_font()
    """
    import matplotlib
...
class Plotter:
    ...
    def animate(
        ...
        **fig_kwargs: Any,
        ...
        """通用动画：逐帧调用 ``update(frame_idx)`` 更新画面。

        ``update`` 接收帧序号，直接在当前 Axes 上修改图形元素并返回它们。

        Args:
            update: ``update(frame_idx) -> list[Artist]``，每帧被调用一次。
            frames: 总帧数（int）或帧数据列表（如 ``[0, 1, 2, ...]``）。
            init: 可选的 ``init() -> list[Artist]``，初始化空白帧。
            interval: 帧间间隔（ms），默认 30。
            figsize: 窗口尺寸 ``(宽, 高)``，单位英寸。
            title: 图表标题。
            xlabel: x 轴标签。
            ylabel: y 轴标签。
            save_path: 输出文件路径（如 ``"outputs/demo.mp4"``）；为 None 时弹窗播放。
            save_dpi: 保存分辨率，默认 150。
            repeat: 是否循环播放，默认 True。
            **fig_kwargs: 透传给 ``plt.subplots()`` 的其他参数。
        ...
        示例::
            ...
            Plotter.animate(update, frames=100, init=init, interval=50)
        """
        import matplotlib
        import matplotlib.pyplot as plt
        ...
        if save_path is not None:
            print(f"正在保存动画至 {save_path} …")
            ani.save(save_path, dpi=save_dpi, writer="ffmpeg")
            print("动画保存完成。")
            plt.close(fig)
